...
設計仕様は `spec/SPEC.md` を単一ソースとする。

- 機能追加・データモデル変更・コンポーネント間の責務変更時は **同一 PR 内で `spec/SPEC.md` を更新** すること
- README はユーザー向け手順、`spec/SPEC.md` は設計判断・データモデル・拡張ポイントを記述。役割を混在させない
- 仕様書のみの軽微修正（誤字等）は `[docs]` プレフィックスで OK
